import { OmegaUp } from '../omegaup';
import { types } from '../api_types';
import T from '../lang';
import { createApp, h, reactive } from 'vue';
import problem_Edit from '../components/problem/Edit.vue';
import * as ui from '../ui';
import * as api from '../api';

OmegaUp.on('ready', () => {
  const payload = types.payloadParsers.ProblemEditPayload();
  if (payload.statusError) {
    ui.error(payload.statusError);
  }
  if (payload.statusSuccess) {
    ui.success(T.problemEditUpdatedSuccessfully);
  }
  const solutions: types.Statements = {
    [payload.statement?.language || 'es']: payload.solution?.markdown || '',
  };
  const state = reactive({
    admins: payload.admins,
    groups: payload.groupAdmins,
    initialTab: window.location.hash
      ? window.location.hash.substring(1)
      : 'edit',
    publishedRevision: payload.publishedRevision,
    statement: payload.statement,
    solution: payload.solution || {
      language: 'es',
      markdown: '',
      images: {},
    },
    problemLevel: payload.problemLevel,
    selectedPublicTags: payload.selectedPublicTags,
    selectedPrivateTags: payload.selectedPrivateTags,
    searchResultUsers: [] as types.ListItem[],
    searchResultGroups: [] as types.ListItem[],
  });

  const refreshProblemAdmins = (): void => {
    api.Problem.admins({ problem_alias: payload.alias })
      .then((data) => {
        state.admins = data.admins;
        state.groups = data.group_admins;
      })
      .catch(ui.apiError);
  };

  createApp({
    render: () =>
      h(problem_Edit, {
        data: payload,
        initialTab: state.initialTab,
        originalVisibility: payload.visibility,
        admins: state.admins,
        groups: state.groups,
        log: payload.log,
        publishedRevision: state.publishedRevision,
        value: state.publishedRevision,
        statement: state.statement,
        solution: state.solution,
        problemLevel: state.problemLevel,
        selectedPublicTags: state.selectedPublicTags,
        selectedPrivateTags: state.selectedPrivateTags,
        searchResultUsers: state.searchResultUsers,
        searchResultGroups: state.searchResultGroups,
        onUpdateProblemLevel: (levelTag?: string) => {
          api.Problem.updateProblemLevel({
            problem_alias: payload.alias,
            level_tag: levelTag,
          })
            .then(() => {
              ui.success(T.problemLevelUpdated);
              state.problemLevel = levelTag;
            })
            .catch(ui.apiError);
        },
        onUpdateMarkdownContents: (
          statements: types.Statements,
          language: string,
          currentMarkdown: string,
          markdownType: string,
        ) => {
          // First update markdown contents to current markdown, otherwise
          // component won't detect any change if two different language
          // solutions are the same.
          if (markdownType === 'statements') {
            state.statement.markdown = currentMarkdown;
            if (Object.prototype.hasOwnProperty.call(statements, language)) {
              state.statement = {
                language: language,
                markdown: statements[language],
                images: {},
                sources: {},
              };
              return;
            }
            api.Problem.details(
              {
                problem_alias: payload.alias,
                statement_type: 'markdown',
                show_solvers: false,
                lang: language,
              },
              { quiet: true },
            )
              .then((response) => {
                if (response.statement.language !== language) {
                  response.statement.markdown = '';
                }
                statements[language] = response.statement.markdown;
                state.statement = response.statement;
              })
              .catch((error) => {
                if (error.httpStatusCode == 404) {
                  return;
                }
                ui.apiError(error);
              });
          } else {
            state.solution.markdown = currentMarkdown;
            if (Object.prototype.hasOwnProperty.call(solutions, language)) {
              state.solution.markdown = solutions[language];
              return;
            }
            api.Problem.solution(
              {
                problem_alias: payload.alias,
                lang: language,
              },
              { quiet: true },
            )
              .then((response) => {
                const solution = response.solution || {
                  language: 'es',
                  markdown: '',
                  images: {},
                };
                if (solution.language !== language) {
                  solution.markdown = '';
                }
                solutions[language] = solution.markdown;
                state.solution = solution;
              })
              .catch((error) => {
                if (error.httpStatusCode == 404) {
                  return;
                }
                ui.apiError(error);
              });
          }
        },
        onAddTag: (alias: string, tagname: string, isPublic: boolean) => {
          api.Problem.addTag({
            problem_alias: alias,
            name: tagname,
            public: isPublic,
          })
            .then(() => {
              ui.success(T.tagAdded);
              if (isPublic) {
                state.selectedPublicTags.push(tagname);
              } else {
                state.selectedPrivateTags.push(tagname);
              }
            })
            .catch(ui.apiError);
        },
        onRemoveTag: (alias: string, tagname: string, isPublic: boolean) => {
          api.Problem.removeTag({
            problem_alias: alias,
            name: tagname,
          })
            .then(() => {
              ui.success(T.tagRemoved);
              // FIXME: For some reason this is not being reactive
              if (isPublic) {
                state.selectedPublicTags = state.selectedPublicTags.filter(
                  (tag) => tag !== tagname,
                );
              } else {
                state.selectedPrivateTags = state.selectedPrivateTags.filter(
                  (tag) => tag !== tagname,
                );
              }
            })
            .catch(ui.apiError);
        },
        onChangeAllowUserAddTag: (
          alias: string,
          title: string,
          allowTags: boolean,
        ) => {
          api.Problem.update({
            problem_alias: alias,
            title: title,
            allow_user_add_tags: allowTags,
            message: `${T.problemEditFormAllowUserAddTags}: ${allowTags}`,
          })
            .then(() => {
              ui.success(T.problemEditUpdatedSuccessfully);
            })
            .catch(ui.apiError);
        },
        onAddAdmin: (username: string): void => {
          api.Problem.addAdmin({
            problem_alias: payload.alias,
            usernameOrEmail: username,
          })
            .then(() => {
              ui.success(T.adminAdded);
              refreshProblemAdmins();
            })
            .catch(ui.apiError);
        },
        onRemoveAdmin: (username: string): void => {
          api.Problem.removeAdmin({
            problem_alias: payload.alias,
            usernameOrEmail: username,
          })
            .then(() => {
              ui.success(T.adminRemoved);
              refreshProblemAdmins();
            })
            .catch(ui.apiError);
        },
        onAddGroupAdmin: (groupAlias: string): void => {
          api.Problem.addGroupAdmin({
            problem_alias: payload.alias,
            group: groupAlias,
          })
            .then(() => {
              ui.success(T.groupAdminAdded);
              refreshProblemAdmins();
            })
            .catch(ui.apiError);
        },
        onRemoveGroupAdmin: (groupAlias: string): void => {
          api.Problem.removeGroupAdmin({
            problem_alias: payload.alias,
            group: groupAlias,
          })
            .then(() => {
              ui.success(T.groupAdminRemoved);
              refreshProblemAdmins();
            })
            .catch(ui.apiError);
        },
        onSelectVersion: (
          selectedRevision: types.ProblemVersion,
          updatePublished: boolean,
        ) => {
          api.Problem.selectVersion({
            problem_alias: payload.alias,
            commit: selectedRevision.commit,
            update_published: updatePublished,
          })
            .then(() => {
              state.publishedRevision = selectedRevision;
              ui.success(T.problemVersionUpdated);
            })
            .catch(ui.apiError);
        },
        onRunsDiff: (
          versions: types.CommitRunsDiff,
          selectedCommit: types.ProblemVersion,
        ) => {
          api.Problem.runsDiff({
            problem_alias: payload.alias,
            version: selectedCommit.version,
          })
            .then((response) => {
              versions.runsDiff[selectedCommit.version] = response.diff;
            })
            .catch(ui.apiError);
        },
        onRemove: (problemAlias: string) => {
          api.Problem.delete({ problem_alias: problemAlias })
            .then(() => {
              window.location.href = '/problem/mine/';
            })
            .catch(ui.apiError);
        },
        onUpdateSearchResultUsers: (query: string) => {
          api.User.list({ query, rowcount: 10 })
            .then(({ results }) => {
              state.searchResultUsers = results.map(
                ({ key, value }: types.ListItem, index) => ({
                  key,
                  value: `${String(index + 1).padStart(2, '0')}.- ${ui.escape(
                    value,
                  )} (<strong>${ui.escape(key)}</strong>)`,
                }),
              );
            })
            .catch(ui.apiError);
        },
        onUpdateSearchResultGroups: (query: string) => {
          api.Group.list({
            query,
          })
            .then((data) => {
              // Groups previously added into the contest should not be
              // shown in the dropdown
              const addedGroups = new Set(
                state.groups.map((group) => group.alias),
              );
              state.searchResultGroups = data
                .filter((group) => !addedGroups.has(group.value))
                .map((group) => ({
                  key: group.value,
                  value: `${ui.escape(group.label)} (<strong>${ui.escape(
                    group.value,
                  )}</strong>)`,
                }));
            })
            .catch(ui.apiError);
        },
      }),
  }).mount('#main-container');
});
