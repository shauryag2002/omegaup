...
import BootstrapVueNext, { BButton, BFormInput } from 'bootstrap-vue-next';
...
describe('Header.vue', () => {
  it('Should contain the header buttons and problem name input', async () => {
    const wrapper = shallowMount(Header, { global: { plugins: [store, BootstrapVueNext] } });
  ...
  it('Should reset the store on clicking the reset button', async () => {
    ...
    const wrapper = mount(Header, { global: { plugins: [store, BootstrapVueNext] } });
